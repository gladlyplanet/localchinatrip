  ...
  return (
    <div className={`grid gap-5 ${compact ? "md:grid-cols-2 xl:grid-cols-4" : "md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
      {items.map((item, index) => {
        ...
        return (
          <Link key={item.slug} href={`/experiences/${item.slug}`} className="group min-w-0 overflow-hidden rounded-lg border hairline bg-white shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-soft">
            ...
            <div className="p-4">
              <h3 className="safe-wrap font-serif text-xl font-semibold leading-7 text-ink">{text.title}</h3>
              <p className="safe-wrap mt-2 min-h-12 text-sm leading-6 text-mist">{text.intro}</p>
              <p className="mt-3 text-xs font-semibold text-moss">{item.locations.length} {ui.locations}</p>
              <span className="safe-wrap mt-4 inline-flex text-sm font-semibold leading-5 text-ink">{ui.view} <span className="ms-2 shrink-0 text-moss">→</span></span>
            </div>
          </Link>
        );
      })}
