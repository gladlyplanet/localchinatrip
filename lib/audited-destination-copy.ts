...
export function getAuditedDestinationMedia(
  ...
  const specificZh: Record<string, { overview: string; experience: string }> = {
    ...
    "成都大熊猫基地": {
      overview: "成都大熊猫基地的重点是看真实的熊猫生活环境和保护工作：竹林步道、不同年龄段的熊猫馆舍、饲喂时段、幼年熊猫照护区和科普展陈共同构成参观价值，而不只是拍一张熊猫照片。",
      experience: "游览应尽量早到，顺着馆区动线安静观察熊猫进食、休息和活动状态，再结合讲解理解繁育研究、栖息地保护和公众教育。遇到人流多时，要预留等待和调整路线的时间。"
    }
  };
